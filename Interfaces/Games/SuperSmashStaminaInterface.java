 ...
 * @author Salinas
 */
public class SuperSmashStaminaInterface implements MainInterface {
    
    public GUITools tools;
    
    public SuperSmashStaminaInterface(){
    ...
    public ArrayList<Team> getDatabaseValues() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                TeamPlayer tm = new TeamPlayer(n, d, 0.0, b, b, "stamina");
                database.add(tm);
    ...
    public void generateRandomInstance(double difTeam, double sucessProb, Team team1 , Team team2, int id, JTextArea Match1, int time) {
        ...
        if (goal_Case == 1) {
            team1.setGoalsteam(team1.getGoalsteam() - (int) (1+Math.floor(Math.random()*25)));
    ...
    public void goalsInstantMatch(double timeelapsed, Team team1, Team team2, JCheckBox CheckTE,
        ...
        while (true) {
           ...
           // System.out.println("Dif1: " + team1.getTeamdiff() + " Dif2: " + team2.getTeamdiff() + " time: " + counter + " " + team1.getGoalsteam() + "-" + team2.getGoalsteam());
            if ((team1.getGoalsteam() < 1 || team2.getGoalsteam() < 1)) {
                break;
            }
        }
        String info = team1.getGoalsteam() + "-" + team2.getGoalsteam();
        if(team1.getGoalsteam() < 0) team1.setGoalsteam(0);
        if(team2.getGoalsteam() < 0) team2.setGoalsteam(0);
