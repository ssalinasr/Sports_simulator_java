 ...
 * @author Salinas
 */
public class MenClubsFootballInterface implements MainInterface {

    public GUITools tools;

    public MenClubsFootballInterface() {
    ...
    public ArrayList<Team> getDatabaseValues() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                TeamFootball tm = new TeamFootball(n, levelteam, 0.0, e, e, a, d, m);
                database.add(tm);
            }
    ...
    public ArrayList<String> getLeagues() throws SQLException {
        ...
        DBConsultas r = new DBConsultas();
        ResultSet rt = r.getLeagues();
        while (rt.next()) {
            String n = rt.getString("team_league");
            leagues.add(n);
        }
        r.cn.desconectar();
        return leagues;
    ...
    public String getClubLeagues(String team) throws SQLException {
        String league = "";
        DBConsultas r = new DBConsultas();
        ResultSet rt = r.getClubTeams();
        while (rt.next()) {
            if(rt.getString("team_name").equals(team)){
            league = rt.getString("team_league");
            break;
            }
        }
        r.cn.desconectar();
    ...
    public ArrayList<Team> getDatabaseValues(String league) {
        ArrayList<Team> database = new ArrayList<>();
        try {
            DBConsultas r = new DBConsultas();
            ResultSet rs = r.getClubTeamsByLeague(league);
            while (rs.next()) {
                String n = rs.getString("team_name");
                int d = rs.getInt("team_def");
                int m = rs.getInt("team_mid");
    ...
    public void generateRandomInstance(double difTeam, double sucessProb, Team team, int id,
        ...
        if (goal_Case == 1) {
            ...
            team.setGlobalgoalsteam(team.getGlobalgoalsteam() + 1);
    ...
    public void goalsInstantMatch(double timeelapsed, Team team1, Team team2, JCheckBox CheckTE,
        ...
        while (counter < timeelapsed) {
            ...
            if (counter == 89 && (team1.getGoalsteam() == team2.getGoalsteam()) && gcounter == 0) {
                if (CheckTE.isSelected()) {
                    timeelapsed += 30;
                }
            }

            if ((isDoubleRound && counter == 89) && (team1.getGlobalgoalsteam()== team2.getGlobalgoalsteam() && gcounter == 2)) {
