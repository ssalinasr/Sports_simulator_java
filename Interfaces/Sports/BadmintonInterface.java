 ...
 * @author Salinas
 */
public class BadmintonInterface implements MainInterface {

    public GUITools tools;

    public BadmintonInterface() {
    ...
    public double getSucessProbability(double difTeam1, double difTeam2) {
        ...
        if (diff < -500000) { //Diferencia menor a -70 (negativa máxima)
            return DefaultValues.BADMINTON_PROB_L1;
        }
        if (diff >= -500000 && diff <= -100000) { //Diferencia entre -60 y 70 (negativa aún más grande)
            return DefaultValues.BADMINTON_PROB_L2;
        }

        if (diff >= -100000 && diff <= -10000) { //Diferencia entre -50 y 60 (negativa muy grande)
            return DefaultValues.BADMINTON_PROB_L3;
        }

        if (diff >= -10000 && diff <= 100000) { //Diferencia entre -50 y -40 (negativa grande)
            return DefaultValues.BADMINTON_PROB_L4;
        }

        if (diff >= 100000 && diff <= 500000) { //Diferencia entre -50 y -40 (negativa grande)
            return DefaultValues.BADMINTON_PROB_L5;
        }

        if (diff > 500000) { //Diferencia entre -40 y -30 (negativa grande)
            return DefaultValues.BADMINTON_PROB_L6;
        }
    ...
    public void goalsInstantMatch(double timeelapsed, Team team1, Team team2, JCheckBox CheckTE,
        ...
        while (true) {
            ...
            if ((team1.getGoalsteam() > 21 || team2.getGoalsteam() > 21) || (team1.getGoalsteam() > 20 && team2.getGoalsteam() - team1.getGoalsteam() < 2)) {
                break;
            }
        }
